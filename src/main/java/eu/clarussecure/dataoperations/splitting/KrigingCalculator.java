...
public class KrigingCalculator {
	...
	public static KrigingResult calculate(String[] x, String[] y, String[] z, String[] c, String p) {
		List<DBRecord> points = IntStream.range(0, x.length)
				.mapToObj(i -> new DBRecord(x[i], y[i], z[i]))
	...
	public static KrigingResult calculateKrigingStep3(List<DBRecord> points, BigDecimal xToCalculate, BigDecimal yToCalculate, List<BigDecimal> calculatedOnCloud){
		...
		BigDecimal maxDistance = sqrt(xToCalculate.add(xVariance).add(xToCalculate).pow(2).add(yToCalculate.add(yVariance).add(yToCalculate).pow(2)));
		BigDecimal minDistance = sqrt(xToCalculate.subtract(xVariance).add(xToCalculate).pow(2).add(yToCalculate.subtract(yVariance).add(yToCalculate).pow(2)));
		...
		for(int i=0;i<pointsWithM0.size();i++)
		{
			for(int j=i;j<pointsWithM0.size();j++)
			{
				...
				if(i!= 0 && distance.compareTo(minDistance) == 1 && distance.compareTo(maxDistance) == -1)
				{
					...
					if(distance.intValue() > furthestDistanceCloud) furthestDistanceCloud = distance.intValue();
				}
                index++;
			}
		}
...
//    System.out.println(zEstimation);
	...
	private static BigDecimal calculateAverage(List<DBRecord> points, String field) {
		...
		while(iterator.hasNext()) {
			...
			switch(field) {
				case "x": avg = avg.add(reg.getX());
				break;
				case "y": avg = avg.add(reg.getY());
				break;
				case "z": avg = avg.add(reg.getZ());
				break;
			}
		}
		...
		for (DBRecord reg : points)
			{
				...
				switch(field) {
					case "x": diff = reg.getX().subtract(avg);
					break;
					case "y": diff = reg.getY().subtract(avg);
					break;
					case "z": diff = reg.getZ().subtract(avg);
					break;
				}
...
//			System.out.println("Result for "+i+": "+result);
	...
	private static void calculateSphericalGamma(List<PointDistance> points) {
		for(int i=0;i<points.size();i++)
		{
			...
			//System.out.println( ((points.get(i).getDistance().multiply(new BigDecimal(3)).divide(new BigDecimal(2*Constants.c))).subtract((points.get(i).getDistance().pow(3).divide(new BigDecimal(2*(Constants.c^3)))))).multiply(new BigDecimal(Constants.a+Constants.b)) );
